...
- **Nest vs Supabase repositories.** `infrastructure/` contains both `Nest*` and `Supabase*` repository implementations. The composition roots select the **Nest** ones by default (`composition/chat.ts`, `composition/products.ts`, `composition/auth.ts` → `Nest*`). The NestJS backend is the source of truth — Supabase repos are opt-in alternates: `composition/auth.ts` only swaps in `SupabaseProfileRepository` when `isSupabaseConfigured()` is true (both `VITE_SUPABASE_URL` and `VITE_SUPABASE_PUBLISHABLE_KEY`/`VITE_SUPABASE_ANON_KEY` set). The Supabase client is created lazily via `getSupabaseClient()`, so importing a `Supabase*` repo never breaks boot. Check the `composition/` file to see which is actually wired before changing behavior.
